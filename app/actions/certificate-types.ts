...
export async function createCertificateType(state: FormState, formData: FormData): Promise<FormState> {
  ...
  const validated = Schema.safeParse({ name: formData.get('name'), description: formData.get('description') })
  ...
  await prisma.certificateType.create({ data: { name: validated.data.name, description: validated.data.description || null } })
...
export async function updateCertificateType(id: string, state: FormState, formData: FormData): Promise<FormState> {
  ...
  await prisma.certificateType.update({
    where: { id },
    data: { name: validated.data.name, description: validated.data.description || null },
  })
  revalidatePath('/dashboard/certificate-types')
  return { success: true }
}
...
export async function deleteCertificateType(id: string) {
  await requireAdmin()
  const events = await prisma.event.count({ where: { certificateTypeId: id } })
  if (events > 0) return
